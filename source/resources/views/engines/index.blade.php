...
@section('content')
    <div class="max-w-6xl mx-auto py-8">
        <div class="flex justify-between items-center mb-6">
            ...
            <a href="{{ url('/engines/create') }}" class="bg-blue-600 text-white px-4 py-2 rounded">Add Engine</a>
        </div>

        <form method="GET" action="{{ url('/engines') }}engines" class="mb-6 flex gap-2">
                ...
                @forelse($engines ?? [] as $engine)
                    <tr class="border-b">
                        ...
                        <td class="p-4 text-right flex justify-end gap-2">
                            <a href="{{ url('/engines/'.  $engine->id) }}" class="text-blue-600">View</a>
                            <form action="{{ url('/engines') }}" method="POST" onsubmit="return confirm('Delete this engine?');">
                                @csrf @method('DELETE')
